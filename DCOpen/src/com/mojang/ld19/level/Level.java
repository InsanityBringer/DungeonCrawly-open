...
public class Level {
        ...
        if (!loading)
        {
	        for (int x = 0; x < xw; x++) 
	        {
	            for (int y = 0; y < xh; y++) 
	            {
	                ...
	                {
	                	int corner = 0;
	                	double posx = (double)x / 2d;
	                	double posy = (double)y / 2d;
	...
	/**
	 * Checks if all special switches are triggered on the world, triggers all tiles with specified id if true
	 * @param id The id of the switches to check
	 */
    ...
    public static Level get(int layer, Player player) {
        ...
        if (level == null) {
            level = new Level(Art.load("/res/map/layer_" + layer + ".png"),Art.load("/res/map/items_" + layer + ".png"), player, layer);
    ...
    public static void loadMap(int layer, Player player, RandomAccessFile loadfile) throws IOException
    {
    	Level level = new Level(Art.load("/res/map/layer_" + layer + ".png"), null, player, layer, true);
    	for (int x = 0; x < level.w; x++)
    	{
    		for (int y = 0; y < level.h; y++)
    		{
    ...
    public static void loadAllOpenLevels(Player player, RandomAccessFile loadfile) throws IOException
    {
